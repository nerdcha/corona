...
forecast_plot_data <- data.frame(day=seq(from=obs_dataframe$Date[1], length.out=n_obs + n_forecast, by="day"),
                                 fitted=c(apply(fitted_values$y_hat[,,2] * population, 2, median), rep(NA, n_forecast)),
                                 projected=c(rep(NA, n_obs), apply(fitted_values$y_forc[,,2]*population, 2, median)),
                                 hi=c(rep(NA, n_obs), apply(fitted_values$y_forc[,,2]*population, 2, quantile, probs=c(0.975))),
                                 lo=c(rep(NA, n_obs), apply(fitted_values$y_forc[,,2]*population, 2, quantile, probs=c(0.025))))
...
forecast_cases_plot_data <- data.frame(day=seq(from=obs_dataframe$Date[1], length.out=n_obs + n_forecast, by="day"),
                                       ...
                                       projected=c(rep(NA, n_obs), apply((fitted_values$y_forc[,,2] + fitted_values$y_forc[,,3])*population, 2, median)),
                                       hi=c(rep(NA, n_obs), apply((fitted_values$y_forc[,,2] + fitted_values$y_forc[,,3])*population, 2, quantile, probs=c(0.975))),
                                       lo=c(rep(NA, n_obs), apply((fitted_values$y_forc[,,2] + fitted_values$y_forc[,,3])*population, 2, quantile, probs=c(0.025))))
